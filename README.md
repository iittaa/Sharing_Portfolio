...
## 作成した背景
私がポートフォリオを作成しようと思った時、**「どんなポートフォリオを作成しよう。」**、**「他の人たちはどんなポートフォリオを作っているのだろう。」**と感じていました。
そんな中私は、作成したポートフォリオを共有できるサイトがあれば何かと便利なのではないかと考えました。  
...
反対に**投稿者**のメリットに関してです。ポートフォリオは一体なんのために作成するものなのかを考えた時に、作成者は必ずこう思います。多くのユーザーに使ってもらいたいと必ず思うと思います。ユーザーに使われなければそのサービスはなんの意味もありません。なので、投稿者は共有サイトに自分の作成しポートフォリオを投稿することで多くのユーザーに広められ、使ってもらえると思ったらです。

実際にどんなポートフォリオを自分が作ったら多くの人に使ってもらえるかを考えた時に、<font color="Red">「これしかない」</font>と思い、Sharing Portfolioを作ることを決意しました。



## こだわりポイント
1. CI/CD パイプラインの構築
circleCIやDockerなどモダンな技術を使用し、CI/CD パイプラインの構築を実装しました。実装したことによって何度も手動で行っていたテストやデプロイを自動で行うこと


2. シンプルなUI
見やすさ、使いやすさを意識したUIにしました。
とにかくシンプルにということを心がけました。  
サイドバーやタグバーを導入することによって見た目のシンプルさを実現しました。  
また、タグ機能に関しては**tag-it**を導入したことによってタグ機能の見栄えにもこだわりました。
